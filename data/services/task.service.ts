...
export async function listTasks(filters?: {
  ...
  return Object.values(tasks).filter((t) => {
    // If filtering by userId, return all statuses for that user
    ...
    if (filters?.area && !t.serviceAddress.toLowerCase().includes(filters.area.toLowerCase()))
      return false;
    return true;
  });
}
