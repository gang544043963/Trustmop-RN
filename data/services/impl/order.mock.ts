...
export async function createOrder(taskId: string, providerId: string): Promise<Order> {
  ...
  const order: Order = {
    id: 'order-' + Date.now(),
    taskId, userId: task.userId, providerId,
    serviceType: task.serviceType, serviceAddress: task.serviceAddress,
    scheduledDate: task.scheduledDate, timeSlot: task.timeSlot,
    agreedPrice: task.budgetMax, platformFeeRate: 0.10,
  ...
  await triggerNotification('task_accepted', { userId: task.userId, providerId, orderId: order.id, taskId });
  return order;
}
...
export async function listOrders(id: string, role: IdentityType): Promise<Order[]> {
  await delay(100);
  const orders = (await getItem<Record<string, Order>>(STORAGE_KEYS.ORDERS)) ?? {};
  return Object.values(orders).filter((o) => role === 'user' ? o.userId === id : o.providerId === id);
}
...
export async function updateOrderStatus(orderId: string, newStatus: OrderStatus, note?: string): Promise<Order> {
  ...
  if (!allowed.includes(newStatus)) throw new Error(`Invalid status transition: ${order.status} → ${newStatus}`);
  const now = new Date().toISOString();
  orders[orderId] = { ...order, status: newStatus, statusHistory: [...order.statusHistory, { status: newStatus, timestamp: now, note }], updatedAt: now };
  await setItem(STORAGE_KEYS.ORDERS, orders);
  if (newStatus === 'pending_confirmation') {
    await triggerNotification('order_pending_confirmation', { userId: orders[orderId].userId, providerId: orders[orderId].providerId, orderId });
  }
...
export async function addCompletionPhotos(orderId: string, photos: string[]): Promise<Order> {
  ...
  const orders = (await getItem<Record<string, Order>>(STORAGE_KEYS.ORDERS)) ?? {};
  if (!orders[orderId]) throw new Error(`Order not found: ${orderId}`);
  orders[orderId] = { ...orders[orderId], completionPhotos: [...orders[orderId].completionPhotos, ...photos], updatedAt: new Date().toISOString() };
  await setItem(STORAGE_KEYS.ORDERS, orders);
  return orders[orderId];
}
...
export async function mockRelease(orderId: string): Promise<Order> {
  ...
  await triggerNotification('payment_completed', { userId: order.userId, providerId: order.providerId, orderId });
  return order;
}
